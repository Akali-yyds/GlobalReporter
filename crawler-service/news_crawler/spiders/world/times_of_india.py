"""
Times of India spider using official RSS feeds.
"""
import logging
...
class TimesOfIndiaSpider(BaseNewsSpider):
    name = "times_of_india"
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "Times of India: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
        ...
        for it in items:
            ...
            pub = (
                it.xpath("string(*[local-name()='pubDate'])").get()
                or it.xpath("string(*[local-name()='updated'])").get()
                or ""
            ).strip()
            ...
            item["license_mode"] = "publisher_public_noncommercial"
            ...
            item["source_metadata"] = {
                ...
                "usage_note": "personal_noncommercial_only",
            }
