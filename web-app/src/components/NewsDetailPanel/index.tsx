...
function toggleFavorite(eventId: string, favorited: boolean) {
  window.dispatchEvent(
    new CustomEvent('favorite-toggle', { detail: { eventId, favorited } })
  );
}
...
const NewsDetailPanel = memo(({ event, isOpen, onClose, onBack, regionHotspots, regionName, onRegionHotspotClick }: NewsDetailPanelProps) => {
            ...
            <ul className="region-news-list">
              {regionHotspots.map((h) => (
                <li
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => e.key === 'Enter' && onRegionHotspotClick?.(h.event_id)}
                >
                  <div className="region-news-meta">
                    <span className="region-news-heat">热度 {h.heat_score}</span>
                    {h.geo_name && <span className="region-news-geo">{h.geo_name}</span>}
                  </div>
                  <span className="region-news-title">{h.title}</span>
                  {h.summary && (
                    <span className="region-news-summary">{h.summary}</span>
                  )}
                </li>
              ))}
            </ul>
  ...
  const articleUrl = d.primary_article_url?.trim();
  const channelLabel = d.primary_source_name || d.primary_source_code || '未知渠道';
    ...
    <div className="news-detail-panel">
      ...
      <div className="detail-content">
        ...
        <article className="detail-article">
          <h1 className="article-title">{d.title}</h1>
          ...
          <div className="article-channel" title="新闻来源渠道">
            ...
            {d.primary_source_code && d.primary_source_name && (
              <span className="channel-code">({d.primary_source_code})</span>
            )}
          </div>

          <div className="article-meta">
            <span className="meta-country">{d.main_country || '未知地区'}</span>
            <span className="meta-separator">|</span>
            <span className="meta-heat">热度: {d.heat_score}</span>
            <span className="meta-separator">|</span>
            <span className="meta-articles">{d.article_count} 篇相关报道</span>
          </div>
          ...
          <div className="article-regions">
            ...
            <div className="region-tags">
              {d.geo_mappings?.map((mapping) => {
                const label = mapping.geo_name || mapping.matched_text || mapping.geo_key;
                const geoType = mapping.geo_type || (mapping.display_type?.includes('country') ? 'country' : mapping.display_type?.includes('city') ? 'city' : 'admin1');
              ...
              {(!d.geo_mappings || d.geo_mappings.length === 0) && (
                <span className="no-region">暂无地区信息</span>
              )}
            </div>
          </div>

          {d.related_sources && d.related_sources.length > 1 && (
            <div className="article-sources-list">
              <h3>报道来源</h3>
              <ul>
                {d.related_sources.map((s) => (
                  <li key={`${s.source_code}-${s.article_url}`}>
                    <button
                      type="button"
                      className="source-link"
                      onClick={() => window.open(s.article_url, '_blank', 'noopener,noreferrer')}
                    >
                      {s.source_name}
                    </button>
                  </li>
