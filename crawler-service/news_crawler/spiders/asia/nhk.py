"""
NHK World News Spider — uses NHK World RSS feeds.
Covers Japan news and Asia-Pacific from Japanese public broadcaster.
"""
import logging
...
class NHKSpider(BaseNewsSpider):

    name = "nhk"
    source_name = "NHK World"
    source_code = "nhk"
    source_url = "https://www3.nhk.or.jp/nhkworld"
    country = "JP"
    language = "en"
    category = "news"
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "NHK: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
