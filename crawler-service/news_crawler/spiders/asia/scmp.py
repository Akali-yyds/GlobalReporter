"""
SCMP Spider — uses South China Morning Post RSS feeds.
Covers Hong Kong, China, Asia, world news from a Chinese perspective.
"""
import logging
...
class ScmpSpider(BaseNewsSpider):
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "SCMP: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
