"""
Financial Times Spider — uses FT RSS feed.
Covers global business, economy, and world news.
"""
import logging
...
class FtSpider(BaseNewsSpider):
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "FT: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
