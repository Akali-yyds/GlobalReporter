"""
RFI Spider — uses RFI RSS feeds (French international broadcaster).
Covers world news from a French perspective.
"""
import logging
...
class RfiSpider(BaseNewsSpider):
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "RFI: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
