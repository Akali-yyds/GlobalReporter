"""
NDTV Spider — uses NDTV RSS feeds.
Covers India news, politics, business, world from India's leading news broadcaster.
"""
import logging
...
class NDTVSpider(BaseNewsSpider):
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "NDTV: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
