"""
NVIDIA official blog RSS spider.
"""
...
class NvidiaOfficialSpider(BaseNewsSpider):
    name = "nvidia_official"
    source_name = "NVIDIA Blog"
    source_code = "nvidia_official"
    ...
    def parse_feed(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        for idx, it in enumerate(root.findall(".//item")):
            ...
            item["freshness_sla_hours"] = 168
            item["license_mode"] = "official_public"
            item["crawled_at"] = datetime.now().isoformat()
            item["language"] = self.language
            item["country"] = self.country
            item["category"] = self.category
            item["heat_score"] = max(1, 72 - idx)
