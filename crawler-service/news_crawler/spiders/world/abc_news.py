"""
ABC News spider using official RSS feeds.
"""
import logging
...
def _feed_config_for_url(url: str) -> dict:
    for feed in FEED_CONFIGS:
        if feed["url"] == url:
            return feed
    return FEED_CONFIGS[0]


class AbcNewsSpider(BaseNewsSpider):
    name = "abc_news"
    ...
    def start_requests(self) -> Iterator[Request]:
        self._candidate_emitted_total = 0
    ...
    def parse_rss(self, response, **kwargs) -> Iterator[NewsArticle]:
        ...
        if not items:
            logger.warning(
                "ABC News: no items from %s. Status=%s body[:300]=%s",
                response.url,
                response.status,
                response.text[:300],
            )
        ...
        candidate_limit = max(self.max_items * 5, self.max_items)
        if not hasattr(self, "_candidate_emitted_total"):
            self._candidate_emitted_total = 0
        ...
        for it in items:
            if self._candidate_emitted_total >= candidate_limit:
                break
            ...
            self._candidate_emitted_total += 1
            yield item
